...
const Validate = () => {
   const formik = useFormik({
    ...
    validationSchema: yup.object({
      email: yup.string().required("Email is require").email("Must be a valid email"),
      password: yup.string().required("Password field cannot be empty").min(6, "Password must be complete"),
      c_password: yup.string().oneOf([yup.ref("password"),null], "Password doesn't match")
    })
   ,onSubmit(values){
      console.log(values);
...
<form onSubmit={formik.handleSubmit}>
      ...
      <label for="floating_email" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Email address</label>
 {
  ...
  <div class="grid md:grid-cols-2 md:gap-6">
    ...
    <div class="relative z-0 w-full mb-6 group">
        <input type="text" name="floating_last_name" id="l_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
